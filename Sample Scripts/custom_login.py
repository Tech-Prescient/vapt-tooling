...
def authenticate(helper, paramsValues, credentials):
    ...
    """
    print "Authenticating via `login` Jython script ..."
    msg = helper.prepareMessage();

    pv = {str(k): str(v) for k, v in dict(paramsValues).items()}
    login_url = pv.get("login_url")
    tenant_id = pv.get("tenant_id")
    ...
    requestUri = URI(login_url, False);
    ...
    msg.getRequestBody().setBody(json.dumps(payload))
...
def getRequiredParamsNames():
    ...
    """
    return jarray.array(["login_url", "tenant_id"], java.lang.String);
